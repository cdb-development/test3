...
{
    // Avoid rendering when minimized or invalid data
    ...
    if (FAILED(device->Map(bd->pVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &vtx_resource)))
        return;
    if (FAILED(device->Map(bd->pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource)))
    {
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        if (!draw_list || !draw_list->VtxBuffer.Data || !draw_list->IdxBuffer.Data)
            continue;
    ...
    // Validate required objects before rendering
    if (!bd->pVertexShader || !bd->pPixelShader || !bd->pInputLayout || !bd->pVertexConstantBuffer)
    {
        // Restore state and return - using a scoped block instead of goto
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Validate offsets to prevent buffer overflow
                UINT final_idx_offset = pcmd->IdxOffset + global_idx_offset;
                INT final_vtx_offset = pcmd->VtxOffset + global_vtx_offset;

                if (final_idx_offset >= (UINT)bd->IndexBufferSize ||
                    final_vtx_offset >= bd->VertexBufferSize ||
                    pcmd->ElemCount > (UINT)bd->IndexBufferSize - final_idx_offset)
                {
                    continue; // Skip invalid draw command
                }
                ...
                if (!texture_srv && bd->pFontTextureView)
                    texture_srv = bd->pFontTextureView; // Fallback to font texture

                if (texture_srv)
                {
                    device->PSSetShaderResources(0, 1, &texture_srv);
                    // Finally, draw with validated parameters
                    device->DrawIndexed(pcmd->ElemCount, final_idx_offset, final_vtx_offset);
                }
            }
        }
